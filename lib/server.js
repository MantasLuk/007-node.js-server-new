...
import { PageCreateBlogPost } from '../pages/CreateBlogPost.js';
    ...
    */
    ...
    req.on('end', async () => {
        ...
        if (hasLoginToken) {
            ...
            const currentBrowser = utils.detectedBrowser(headers['user-agent']);
            if (isValidToken) {
                const expireValue = tokenObj.expire;
                if (expireValue && expireValue > Date.now() && tokenObj.browser === currentBrowser) {
                    dataForHandlers.user.isLoggedIn = true;
                    dataForHandlers.user.email = tokenObj.email;
                }
            }
        }
        ...
        if (isPage) {
            ...
            const trimmedPathParts = trimmedPath.split('/');
            if (trimmedPathParts[0] === 'blog' 
                && typeof trimmedPathParts[1] === 'string'
                && trimmedPathParts[1].trim() !== '') {
                pageClass = BlogPost;
            }
